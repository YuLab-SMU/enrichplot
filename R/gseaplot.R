...
##
## This helper prepares one term's horizon bands.
hplot_horizon_group <- function(data, horizonscale = 4) {
    score <- as.numeric(data$runningScore)
    finite <- is.finite(score)
    if (!any(finite)) {
        return(data.frame())
    }

    origin <- min(score[finite])
    score_range <- max(score[finite]) - origin
    if (!is.finite(score_range) || score_range <= 0) {
        score_range <- 1
    }
    cutpoints <- origin + score_range * seq_len(horizonscale) / horizonscale

    bands <- lapply(seq_len(horizonscale), function(i) {
        lower <- if (i == 1) origin else cutpoints[i - 1]
        upper <- cutpoints[i]
        value <- ifelse(
            finite & score > lower,
            (pmin(score, upper) - lower) / (upper - lower),
            0
        )
        ...
        data.frame(
            x = data$x,
            ymin = 0,
            ymax = pmax(0, pmin(1, value)),
            band = paste0("ypos", i),
            stringsAsFactors = FALSE
        )
    })
    result <- do.call(rbind, bands)
    result$Description <- as.character(data$Description[1])
...
hplot_horizon_data <- function(data, horizonscale = 4) {
    groups <- split(data, as.character(data$Description), drop = TRUE)
    result <- lapply(groups, hplot_horizon_group, horizonscale = horizonscale)
    result <- result[vapply(result, nrow, integer(1)) > 0]
    if (length(result) == 0) {
        return(data.frame())
    }
    ...
    result$band <- factor(
        result$band,
        levels = paste0("ypos", seq_len(horizonscale))
    )
    result
}
...
hplot <- function(x, geneSetID, layer = NULL) {
    ...
    horizon_data <- hplot_horizon_data(gsdata, horizonscale = 4)
    horizon_colors <- grDevices::hcl.colors(
        4,
        palette = "BluGrn",
        rev = TRUE
    )
    names(horizon_colors) <- paste0("ypos", seq_along(horizon_colors))
    ...
    ggplot(gsdata, aes(.data$x, .data$runningScore)) +
        ...
        scale_fill_manual(values = horizon_colors, drop = FALSE) +
        theme_minimal() +
        theme(
