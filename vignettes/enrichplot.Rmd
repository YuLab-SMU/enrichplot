...
N
dotplot(ego, showCategory=30, x = ~Count/N) + ggplot2::xlab("Rich Factor")
```
...
```{r fig.width=12, fig.height=8}
gseaplot2(kk, geneSetID = 1:3, pvalue_table = TRUE,
...
```{r fig.width=12, fig.height=4}
gseaplot2(kk, geneSetID = 1:3, subplots = 1)
```

```{r fig.width=12, fig.height=8}
gseaplot2(kk, geneSetID = 1:3, subplots = 1:2)
```
